...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Handle signature saving via canvas data URL
...
<div class="row g-3 mb-3">
    <div class="col-12 col-md-4">
        <section class="card border-0 border-start border-4 shadow-sm position-relative h-100" style="border-left-color: var(--bs-warning) !important;">
            <div class="card-body d-flex align-items-center">
                <div class="me-3 text-warning fs-4 d-flex align-items-center justify-content-center"><i class="fas fa-clock"></i></div>
                <div>
    ...
    <div class="col-12 col-md-4">
        <section class="card border-0 border-start border-4 shadow-sm position-relative h-100" style="border-left-color: var(--bs-success) !important;">
            <div class="card-body d-flex align-items-center">
                <div class="me-3 text-success fs-4 d-flex align-items-center justify-content-center"><i class="fas fa-check-circle"></i></div>
                <div>
    ...
    <div class="col-12 col-md-4">
        <section class="card border-0 border-start border-4 shadow-sm position-relative h-100" style="border-left-color: var(--bs-danger) !important;">
            <div class="card-body d-flex align-items-center">
                <div class="me-3 text-danger fs-4 d-flex align-items-center justify-content-center"><i class="fas fa-times-circle"></i></div>
                <div>
            ...
            <table class="table align-middle table-hover mb-0" id="leaveRequestsTable">
                ...
                <?php else: ?>
                    <?php foreach ($facultyGroupedRequests as $fId =>$group): ?>
                        <?php
                        $facultyName = $group['faculty_name'];$requests = $group['requests'];$totalFacultyRequests = count($requests);$pendingFacultyRequests = 0;
                        $allRefs = [];
                        $allTypes = [];$allStatuses = [];

                        foreach ($requests as$r) {
                            $stRaw = strtolower(trim($r['status'] ?? 'pending'));
                            if ($stRaw === 'pending') $pendingFacultyRequests++;$allRefs[] = strtolower($r['request_ref'] ?? ('lr-' .$r['id']));
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body p-4">
                <div class="mb-4">
                    <h6 class="fw-bold text-body mb-2"><i class="fas fa-chart-pie text-primary me-2"></i> Semester Leave Balance Pool (7 Days Max)</h6>
                    <div class="p-3 border rounded bg-body-tertiary">
                        ...
                        <div class="progress" style="height: 10px;">
                            <div id="modal-pool-progress" class="progress-bar bg-primary" role="progressbar" style="width: 0%;"></div>
                        </div>
                    </div>
                </div>
