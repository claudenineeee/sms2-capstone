...
class LeaveBalance
{
    ...
    public function forFaculty(int $facultyId, string $academicYear): array
    {
        ...
        // Self-heal: if the row violates gender rules, or is missing the
...
<style>
    #new-total-days-hint.text-danger {
        font-weight: 600;
    }

    #new-total-days-hint.text-success {
        font-weight: 500;
    }
</style>
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    ...
    <?php else: ?>
        <button class="btn btn-outline-secondary rounded-pill px-3 shadow-sm d-flex align-items-center gap-2 fw-medium"
            disabled>
...
<div class="row g-3 mb-4">
        ...
        <section class="card stat-card primary border shadow-sm position-relative overflow-hidden h-100">
            ...
            <div class="card-body d-flex align-items-center ps-4">
                ...
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="color: #0d6efd;"><?= $totalAvailable + $totalUsedDays ?> <small
        ...
        <section class="card stat-card warning border shadow-sm position-relative overflow-hidden h-100">
            ...
            <div class="card-body d-flex align-items-center ps-4">
                ...
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="color: #fd7e14;"><?= $totalUsedDays ?> <small
                            class="text-muted fs-6">days used</small></h4>
                </div>
            </div>
        </section>
    </div>
    ...
    <div class="col-12 col-sm-6 col-xl-3">
        <section class="card stat-card success border shadow-sm position-relative overflow-hidden h-100" role="button"
            ...
            <div class="card-body d-flex align-items-center ps-4">
                <div class="stat-icon me-3 fs-4" style="color: #28a745;"><i class="fas fa-battery-three-quarters"></i>
                </div>
                <div class="flex-grow-1">
                    <h6 class="text-muted mb-0 small text-uppercase fw-bold">Remaining Balance</h6>
                    <h4 class="mb-0 fw-bold" style="color: #28a745;"><?= $totalAvailable ?> <small
                            class="text-muted fs-6">days left</small></h4>
                </div>
        ...
        <section class="card stat-card danger border shadow-sm position-relative overflow-hidden h-100">
            ...
            <div class="card-body d-flex align-items-center ps-4">
                ...
                <div>
                    ...
                    <h4 class="mb-0 fw-bold" style="color: #dc3545;"><?= $pendingCount + $documentRequiredCount ?>
                        <small class="text-muted fs-6">requests</small>
                    </h4>
                </div>
            </div>
        </section>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light border-bottom border-light-subtle">
                                <tr>
                                    ...
                                    <th class="pe-4 text-uppercase small text-secondary fw-semibold" style="width: 160px;">
                                        Usage</th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <td class="pe-4">
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <small class="text-muted"
                                                    style="font-size: 11px; min-width: 40px; text-align: right;">
                                                    <?= $pct ?>%
                                                </small>
                                            </div>
                                        </td>
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div
        class="card-header bg-transparent border-bottom border-light-subtle py-3 px-4 d-flex justify-content-between align-items-center">
                    ...
                    <?php else: ?>
                        <?php foreach ($leaveRequests as $row):
                            ...
                            <tr>
                                ...
                                <td><span
                                        class="badge border rounded-pill px-2.5 py-1.5 <?= $badgeClass ?>"><?= htmlspecialchars($statusText, ENT_QUOTES, 'UTF-8') ?></span>
                                </td>
                ...
                <form id="leaveRequestForm" method="post" enctype="multipart/form-data" novalidate>
                    <div class="modal-body p-4">
                            ...
                            <select name="leave_type" id="new-leave-type" class="form-select bg-light" required>
                                ...
                                <?php foreach ($activeLeaves as $lv): ?>
                                    <option value="<?= htmlspecialchars($lv['label']) ?>">
                                        ...
                                        — <?= $lv['balance'] ?> of <?= $lv['total'] ?> days remaining
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                <label class="form-label small fw-medium">Start Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" name="start_date" id="new-start-date" class="form-control bg-light"
                                    required>
                            </div>
                            <div class="col-6">
                                <label class="form-label small fw-medium">End Date <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="reason" class="form-control bg-light" rows="3"
                                placeholder="Provide details regarding your request..." required></textarea>
                        </div>
                        <div class="mb-3">
                        ...
                        <div
                            class="p-3 bg-primary bg-opacity-10 rounded-3 d-flex align-items-center gap-2 text-primary small">
                            <i class="fas fa-info-circle fs-6"></i>
                            <span id="new-total-days-hint">Select a leave type and dates to see the impact on your
                                balance.</span>
                        </div>
                    </div>
                    <div class="modal-footer border-top-0 px-4 pb-4 pt-0">
            ...
            <form id="editLeaveRequestForm" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="leave_type" id="edit-leave-type" class="form-select bg-light" required>
                            <?php foreach ($activeLeaves as $lv): ?>
                                <option value="<?= htmlspecialchars($lv['label']) ?>"><?= htmlspecialchars($lv['label']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label small fw-medium">Start Date <span
                                    class="text-danger">*</span></label>
                            <input type="date" name="start_date" id="edit-start-date" class="form-control bg-light"
                                required>
                        </div>
                        <div class="col-6">
                            <label class="form-label small fw-medium">End Date <span
                                    class="text-danger">*</span></label>
                            <input type="date" name="end_date" id="edit-end-date" class="form-control bg-light"
                                required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-medium">Reason <span class="text-danger">*</span></label>
                        <textarea name="reason" id="edit-reason" class="form-control bg-light" rows="3"
                            required></textarea>
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer border-top-0 px-4 pb-4 pt-0">
                    ...
                    <button type="submit" name="update_leave"
                        class="btn btn-warning rounded-pill px-4 text-dark fw-bold">Update & Resubmit</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body p-4">
                ...
                <div id="modal-approval-box"
                    class="p-3 bg-success bg-opacity-10 border border-success border-opacity-25 rounded-3 mb-3 d-none">
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div id="modal-download-wrapper" class="d-none">
                        <a href="#" id="modal-download-btn" target="_blank"
                            class="btn btn-success btn-sm rounded-pill px-3 fw-bold text-white shadow-sm">
...
<script>
    ...
    /* =========================================================
       SHARED TOAST HELPER — reused by PHP-driven messages and
    ...
    /* =========================================================
       DOM READY — validation, live hints, submit interception
       ========================================================= */
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateHint() {
            ...
            // Reset any previous red/green styling
            ...
            // No balance cap for this category — just show days
            if (remaining === null) {
                if (hint) {
        ...
        // Intercept submit — block if invalid and fire a toast
        const leaveForm = document.getElementById('leaveRequestForm');
        if (leaveForm) {
            leaveForm.addEventListener('submit', function (ev) {
                // Re-run validation one more time to be certain it's current
                updateHint();
        ...
        // Handle PHP-driven messages (formError / formSuccess)
