...
function smsEnsureSecurityTables(): void
{
    ...
    try {
        ...
        if ($hasUsers) {
            try {
                $pdo->exec('ALTER TABLE security_otps ADD CONSTRAINT fk_otp_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE');
            } catch (Throwable $e) {
        ...
        if ($hasUsers) {
            try {
                $pdo->exec('ALTER TABLE password_reset_requests ADD CONSTRAINT fk_prr_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE');
            } catch (Throwable $e) {
                // Ignore if constraint already exists
            }
            try {
                $pdo->exec('ALTER TABLE password_reset_requests ADD CONSTRAINT fk_prr_admin FOREIGN KEY (admin_id) REFERENCES users(id) ON DELETE SET NULL');
            } catch (Throwable $e) {
                // Ignore if constraint already exists
            }
        }
...
function smsRolesForModule(string $moduleKey): array
{
    ...
    $map = [
        ...
        'faculty'            => ['hr', 'adviser', 'panel', 'superadmin', 'admin'],
