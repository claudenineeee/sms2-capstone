...
function smsNotificationResearchGroupDetail(): ?array
{
    ...
    try {
        $stmt = $crad->prepare(
            "SELECT g.id AS research_group_id, p.proposal_number, p.research_title,
                    g.group_number, g.group_name, g.status, g.created_at, g.date_assigned
    ...
    try {
        $assignmentStmt = $crad->prepare(
            "SELECT
                ...
                (
                    SELECT GROUP_CONCAT(panel_name ORDER BY assigned_at ASC, id ASC SEPARATOR ', ')
                    FROM research_panel_assignments
                    WHERE assignment_status = 'Assigned'
                      AND (research_group_id = :panel_group_id OR group_number = :panel_group_number OR proposal_number = :panel_proposal_number)
                ) AS assigned_panels"
        );
        $assignmentStmt->execute([
            ...
            ':panel_group_id' => (int) ($group['research_group_id'] ?? 0),
            ':panel_group_number' => (string) ($group['group_number'] ?? ''),
            ':panel_proposal_number' => (string) ($group['proposal_number'] ?? ''),
        ]);
    ...
    return [
        'title' => 'Research Approval Status',
        'badge' => $status,
        'badge_class' => 'success',
        'icon' => 'fa-users',
        ...
        'details' => [
            ['label' => 'Proposal Number', 'value' => (string) ($group['proposal_number'] ?? '')],
            ['label' => 'Research Group', 'value' => (string) ($group['group_number'] ?? '')],
            ['label' => 'Group Name', 'value' => (string) ($group['group_name'] ?? '')],
            ['label' => 'Status', 'value' => $status],
            ['label' => 'Adviser', 'value' => (string) (($assignment['assigned_adviser'] ?? '') ?: 'For assignment')],
            ['label' => 'Panel Members', 'value' => (string) (($assignment['assigned_panels'] ?? '') ?: 'For assignment')],
        ],
